...
WRAPPER(GObject, constructor, void*, (int type, uint32_t n_construct_properties, void* construct_properties), "iup", type, n_construct_properties, construct_properties);
...
WRAPPER(GObject, dispatch_properties_changed, void*, (int type, uint32_t n_pspecs, void* pspecs), "iup", type, n_pspecs, pspecs);
WRAPPER(GObject, notify, void*, (int type, void* pspecs), "ip", type, pspecs);
...
// wrapper x86 -> natives of callbacks
WRAPPER(GInitiallyUnowned, constructor, void*, (int type, uint32_t n_construct_properties, void* construct_properties), "iup", type, n_construct_properties, construct_properties);
...
WRAPPER(GInitiallyUnowned, dispatch_properties_changed, void*, (int type, uint32_t n_pspecs, void* pspecs), "iup", type, n_pspecs, pspecs);
WRAPPER(GInitiallyUnowned, notify, void*, (int type, void* pspecs), "ip", type, pspecs);
...
// ----- GtkLabel2Class ------
// wrapper x86 -> natives of callbacks
WRAPPER(GtkMisc2, move_cursor, void, (void* label, int step, int count, int extend_selection), "piii", label, step, count, extend_selection);
WRAPPER(GtkMisc2, copy_clipboard, void, (void* label), "p", label);
WRAPPER(GtkMisc2, populate_popup, void, (void* label, void* menu), "pp", label, menu);
WRAPPER(GtkMisc2, activate_link, int, (void* label, void* uri), "pp", label, uri);
...
WRAPPER(GtkEntry2, delete_from_cursor, void, (void* entry, int type, int count), "pii", entry, type, count);
...
void* unwrapCopyGTKClass(void* klass, int type)
{
    ...
    printf_log(LOG_DEBUG, "unwrapCopyGTKClass(%p, %d (%s))\n", klass, type, g_type_name(type));
