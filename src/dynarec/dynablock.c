...
static dynablock_t* internalDBGetBlock(x86emu_t* emu, uintptr_t addr, uintptr_t filladdr, int create, int need_lock)
{
    ...
    if(block && (block->x86_size || (!block->x86_size && !block->done))) {
        ...
        // fill-in jumptable
        if(!addJumpTableIfDefault(block->x86_addr, block->block)) {
    ...
    dynarec_log(LOG_DEBUG, " --- DynaRec Block created @%p:%p (%p, 0x%x bytes)\n", (void*)addr, (void*)(addr+((block)?block->x86_size:0)), (block)?block->block:0, (block)?block->size:0);
