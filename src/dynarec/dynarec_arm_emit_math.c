...
// emit INC8 instruction, from s1 , const c, store result in s1 using s3 and s4 as scratch, with save_s4 is s4 need to be saved
...
void emit_sbb32(dynarec_arm_t* dyn, int ninst, int s1, int s2, int s3, int s4)
{
    ...
    MOVS_REG_LSR_IMM5(s3, s3, 1);       // load into ARM CF
    IFX(X_ALL) {
        SBCS_REG_LSL_IMM8(s1, s1, s2, 0);
    } else {
        SBC_REG_LSL_IMM8(s1, s1, s2, 0);
    }
    IFX(X_PEND) {
        STR_IMM9(s1, xEmu, offsetof(x86emu_t, res));
    }
...
void emit_sbb8c(dynarec_arm_t* dyn, int ninst, int s1, int c, int s3, int s4)
{
    ...
    IFX(X_AF|X_OF|X_CF) {
        ORR_IMM8(s3, s4, c, 0);             // s3 = ~op1 | op2
        AND_REG_LSL_IMM5(s4, s4, c, 0);    // s4 = ~op1 & op2
        AND_REG_LSL_IMM5(s3, s3, s1, 0);   // s3 = (~op1 | op2) & res
        ORR_REG_LSL_IMM8(s3, s3, s4, 0);   // s3 = (~op1 & op2) | ((~op1 | op2) & res)
        IFX(X_CF) {
            UBFX(s4, s3, 7, 1);
