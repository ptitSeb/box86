...
#define CHANGE(A)    \
    ...
    CH(VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DRM_PROPERTIES_EXT, "uPiiUUUU", A)         \
    CH(VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SAMPLE_LOCATIONS_PROPERTIES_EXT, "uPiSuuffui", A)      \
...
SUPER()
#undef GO
static void* find_vkDebugReportCallback_Fct(void* fct)
{
    if(!fct) return fct;
    if(GetNativeFnc((uintptr_t)fct))  return GetNativeFnc((uintptr_t)fct);
    #define GO(A) if(my_vkDebugReportCallback_fct_##A == (uintptr_t)fct) return my_vkDebugReportCallback_##A;
    SUPER()
    #undef GO
    #define GO(A) if(my_vkDebugReportCallback_fct_##A == 0) {my_vkDebugReportCallback_fct_##A = (uintptr_t)fct; return my_vkDebugReportCallback_##A; }
    SUPER()
    #undef GO
    printf_log(LOG_NONE, "Warning, no more slot for ssl3 vkDebugReportCallback callback\n");
    return NULL;
}
static void* reverse_vkDebugReportCallback_Fct(void* fct)
{
    if(!fct) return fct;
    if(GetNativeFnc((uintptr_t)fct))  return GetNativeFnc((uintptr_t)fct);
    return NULL;
    #define GO(A) if(fct == (void*)my_vkDebugReportCallback_##A) return (void*)my_vkDebugReportCallback_fct_##A;
    SUPER()
    #undef GO
}
#undef SUPER
...
static my_vkhead_t* alignFromx86(my_vkhead_t* src, uint32_t cnt);
        ...
        *save = (void*)psrc;
        ...
        while(*p) {
            switch (*p) {
                ...
                case 'P':
                    ++c;
                    *(pdst++) = (uintptr_t)VulkanFromx86((void*)*(psrc++));
                    break;
                case 'Q':
                    ++c;
                    q = *(psrc-1);
                    *(pdst++) = (uintptr_t)alignFromx86((void*)*(psrc++), q);
                    break;
                case 'b':   // debug callback
                    ++c;
                    *(pdst++) = (uintptr_t)find_vkDebugReportCallback_Fct((void*)*(psrc++));
                    break;
                case 'S':   // start of a struct inside struct. Just align, no copy
                    if(c&1) {   //align?
                        ++c;
...
void* vkunalignStruct(void* src, const char* desc, int cnt)
{
    ...
    uint32_t* psrc = (uint32_t*)src;
    int count = (cnt)?cnt:1;
    void** save = (void**)(src+sz*count);
    uint32_t* pdst = (uint32_t*)*save;
    void* ret = *save;
    while(count) {
        const char* p = desc;
        while(*p) {
            switch (*p) {
                ...
                case 'P':
                    ++c;
                    *(pdst++) = (uintptr_t)VulkanTox86((void*)*(psrc++));
                    break;
                case 'Q':
                    ++c;
                    q = *(psrc-1);
                    *(pdst++) = (uintptr_t)unalignTox86((void*)*(psrc++), q);
                    break;
                case 'b':   // debug callback
                    ++c;
                    *(pdst++) = (uintptr_t)reverse_vkDebugReportCallback_Fct((void*)*(psrc++));
                    break;
                case 'S':   // start of a struct inside struct. Just align, no copy
                    if(c&1) {   //align?
                        ++c;
...
void* VulkanTox86(void* src)
{
    if(!src)
        return src;
    return unalignTox86((my_vkhead_t*)src, 1);
}
