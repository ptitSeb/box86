...
int getSymbolInMaps(library_t*lib, const char* name, int noweak, uintptr_t *addr, uint32_t *size)
{
    ...
    if (k!=kh_end(lib->mysymbolmap)) {
        ...
        *addr = AddBridge(lib->priv.w.bridge, lib->context, kh_value(lib->mysymbolmap, k), symbol, 0);
        *size = sizeof(void*);
        return 1;
    }
    ...
    if (k!=kh_end(lib->stsymbolmap)) {
        ...
        if(!symbol) {
            ...
            AddOffsetSymbol(lib->context->maplib, symbol, name);
        *addr = AddBridge(lib->priv.w.bridge, lib->context, kh_value(lib->stsymbolmap, k), symbol, 4);    // all of this for this little "4"
        *size = sizeof(void*);
        return 1;
    }
    // check in symbolmap
    ...
    if (k!=kh_end(lib->symbolmap)) {
        ...
        if(!symbol) {
            ...
            AddOffsetSymbol(lib->context->maplib, symbol, name);
        *addr = AddBridge(lib->priv.w.bridge, lib->context, kh_value(lib->symbolmap, k), symbol, 0);
        *size = sizeof(void*);
        return 1;
    }
    if(!noweak) {
        ...
        if (k!=kh_end(lib->wsymbolmap)) {
            ...
            if(!symbol) {
                ...
                AddOffsetSymbol(lib->context->maplib, symbol, name);
            *addr = AddBridge(lib->priv.w.bridge, lib->context, kh_value(lib->wsymbolmap, k), symbol, 0);
            *size = sizeof(void*);
            return 1;
        }
    }
    ...
    if (k!=kh_end(lib->symbol2map)) 
        if(!noweak || !kh_value(lib->symbol2map, k).weak)
        {
            ...
            if(!symbol) {
                ...
                AddOffsetSymbol(lib->context->maplib, symbol, name);
            *addr = AddBridge(lib->priv.w.bridge, lib->context, kh_value(lib->symbol2map, k).w, symbol, 0);
            *size = sizeof(void*);
            return 1;
        }
