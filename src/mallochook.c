...
/*
 ...
 Problem is, box86 is already loaded in memoery, and already using system libc malloc for many of it's things
    ...
    if(malloc_hack_2)
        if(getMmapped((uintptr_t)p) || (!p && ALLOC && real_realloc)) {
            ...
            if(real_realloc && ALLOC) {
                ...
                while(s && !(getProtection((uintptr_t)p+s)&PROT_READ)) {if(s>box86_pagesize) s-=box86_pagesize; else s=0;}
                memcpy(ret, p, s);
