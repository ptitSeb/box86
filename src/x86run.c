...
x86emurun:
    ...
    while (1)
    {
        ...
        switch(opcode) {
            ...
            case 0xB7:
                tmp8u = opcode - 0xB0;
                emu->regs[tmp8u%4].byte[tmp8u/4] = nextop;
                R_EIP++;
                break;
            ...
            case 0xEB:                      /* JMP Ib */
                tmp32s = (int8_t)nextop; // jump is relative
                R_EIP += tmp32s + 1;
                break;
            case 0xF0:                      /* LOCK */
                break;
